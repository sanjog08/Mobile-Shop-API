...
async function singleImg(req, res) {
    try {
        const mobile = await Mobile.findById(req.params.id);
        if (!mobile || !mobile.file) {
            return res.status(404).send('Image not found');
        }
        // const base64File = mobile.file.toString('base64');
        // res.send(base64File);
        res.send(mobile.file.toString('base64'));
    } catch (err) {
        console.error(err);
...
module.exports = {index, uploads, getMobile, getAllMobiles, oneMobilePage, singleImg}